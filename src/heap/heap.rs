...
impl Heap {
    ...
    pub fn insert(&mut self, x: i32) -> bool {
        // 堆已满
        if self.capacity as i32 == self.count {
            return false;
        }
    ...
    pub fn heapify(&mut self) {
        ...
        loop {
            let mut max_pos = idx;
            if (2 * idx + 1) as i32 <= self.count && self.data[idx] < self.data[2 * idx + 1] {
                max_pos = 2 * idx + 1;
            }
            if (2 * idx + 2) as i32 <= self.count && self.data[max_pos] < self.data[2 * idx + 2] {
                max_pos = 2 * idx + 2;
            }
